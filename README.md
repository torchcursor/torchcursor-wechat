...
- **全文底色用「牺牲壳」双层包裹**：实测粘贴时编辑器只丢弃最外层那一个容器，第二层的底色 `<section>` 能像头部卡片一样存活（机制推导 + 真机复核，`--page-bg none` 可关闭）
- **布局一律不用表格**：实测微信编辑器会把布局表格转成带边框、列宽错乱的表格组件；图文并排用右浮动图片 + 文字绕排，分节标题用纯段落堆叠，图片占位不带边框
...
- **最外层容器只丢一层**。1.1.0 单层包裹全文底色，真机粘贴后底色丢——但当时包在里面的头部卡片背景是活的。三次实测拼出机制：编辑器粘贴时只解包最外层那一个容器。1.2.0 据此改成牺牲壳双层包裹。
- **间隙露白 = margin 不吃背景**。1.2.0 双层包裹后底色进去了，但段落间隙还是白的——底色被摊到每个文字块上，块间 margin 不属于任何块。1.3.0 起所有间距改走 padding、每个块单独补底色，全文无白缝。
- **布局表格 = 被转成表格组件**。头部卡片的图文表格，粘贴后标题、图片全被框上方框。布局一律不用表格。
- **图片尺寸不依赖父容器百分比**：`width:100%` 会随分栏宽度被微信洗掉而塌小（实测 1.5.x），一律用固定 px 或 `max-width:100%`。
